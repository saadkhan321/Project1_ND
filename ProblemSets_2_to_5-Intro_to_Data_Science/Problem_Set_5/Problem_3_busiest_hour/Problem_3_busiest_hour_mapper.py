...
def mapper():
    ...
    for line in sys.stdin: # reading the input file line by line
        
    
        data = line.strip().split(",") # splitting entries as they are comma seperated
        if len(data) != 22 or data[1] == 'UNIT': # condition for the header row and abnormal row entry
            continue
        print "{0}\t{1}\t{2}\t{3}".format(data[1], data[6], data[2], data[3]) # producing mapper output as required for reducer
