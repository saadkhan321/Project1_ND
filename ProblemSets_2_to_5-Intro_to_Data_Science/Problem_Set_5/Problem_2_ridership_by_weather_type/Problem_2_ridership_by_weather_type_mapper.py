...
def mapper():
    ...
    def format_key(fog, rain): # def to create fog-rain scenario pairs
        return '{}fog-{}rain'.format(
            '' if fog else 'no',
            '' if rain else 'no'
        )

    for line in sys.stdin: # reading input line by line
        data = line.strip().split(",") # seperating on the basis of comma
        if len(data) != 22 or data[1] == 'UNIT': # checking for header row and abnormal row entries
            continue
       
        for d in data:
            fog_rain = format_key(float(data[14]), float(data[15])) # pass fog and rain to create fog-rain pair
        
            print "{0}\t{1}".format(fog_rain, data[6]) # producing mapper output with fog-rain pairs and hourly entries
